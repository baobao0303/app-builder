...
export class Compressed extends Compiler {
  ...
  document(node: Document) {
    ...
    return this.emit(doc) + this.emit('{') + this.mapVisit(node.rules) + this.emit('}');
  }
  ...
  keyframes(node: KeyFrames) {
    ...
    return this.emit(head) + this.emit('{') + this.mapVisit(node.keyframes) + this.emit('}');
  }
  ...
  page(node: Page) {
    ...
    return (
      this.emit(`@page ${sel}`) + this.emit('{') + this.mapVisit(node.declarations) + this.emit('}')
    );
  }

  ['font-face'](node: FontFace) {
    return (
      this.emit('@font-face') + this.emit('{') + this.mapVisit(node.declarations) + this.emit('}')
    );
  }

  host(node: Host) {
    return this.emit('@host') + this.emit('{') + this.mapVisit(node.rules) + this.emit('}');
  }
